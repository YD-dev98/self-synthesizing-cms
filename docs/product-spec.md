...
- **Versioned snapshots** — Every time `site_state` changes — whether from intent processing or a TTL expiry sweep — a new version is registered in `site_versions` and the block-level data is captured in `site_state_history`. The version registry (`site_versions`) records *which intent* produced the version (or NULL for system-initiated sweeps). The block data (`site_state_history`) is keyed to the version and may be empty — an empty surface is a valid, recorded state.

To reconstruct the site at any point: query `site_versions` for the version, then `SELECT * FROM site_state_history WHERE site_version = N`. An empty result with an existing version means the surface was intentionally empty. You can trace the full chain: intent `abc-def` produced site version `n`, which changed the site from version `n-1` — for example by adding a weather block and updating a trends card. Or version `n` has no intent, meaning the system swept stale content. Intent IDs and site versions are independent sequences (UUIDs vs. integers), linked through the `intent_id` foreign key on `site_versions`.
